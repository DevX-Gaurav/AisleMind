...
// import { Sprout, KeyRound, Mail, Eye, EyeOff } from "lucide-react";
// import { toast } from "sonner";

// type Mode = "login" | "signup";
// type PendingAction =
//   | { kind: "login"; email: string; password: string; otp: string }
//   | { kind: "signup"; data: { name: string; email: string; password: string; role: Role; storeName?: string }; otp: string }
//   | { kind: "reset"; email: string; otp: string }
//   | null;

// const DEMO_CREDS = [
//   { role: "Admin", email: "admin@aislemind.co", password: "Admin@123" },
//   { role: "Vendor", email: "maren@greenfield.co", password: "Vendor@123" },
//   { role: "Vendor", email: "theo@atelierwest.co", password: "Vendor@123" },
//   { role: "Customer", email: "anika@example.com", password: "Customer@123" },
//   { role: "Customer", email: "jonas@example.com", password: "Customer@123" },
// ];
...
//     name: "", email: "", password: "", role: initialRole as Role, storeName: "",
//   });
...
//     const target = users.find(u => u.email.toLowerCase() === loginForm.email.toLowerCase());
//     if (!target) { toast.error("No account with this email"); return; }
//     if (target.password && target.password !== loginForm.password) { toast.error("Incorrect password"); return; }
//     const otp = generateOtp();
//     localStorage.setItem("aislemind:otp", JSON.stringify({ email: loginForm.email, otp, kind: "login", at: Date.now() }));
//     setPending({ kind: "login", email: loginForm.email, password: loginForm.password, otp });
//     setOtpInput("");
//   };
...
//     }
//     const otp = generateOtp();
//     localStorage.setItem("aislemind:otp", JSON.stringify({ email: signupForm.email, otp, kind: "signup", at: Date.now() }));
...
//     if (!users.some(u => u.email.toLowerCase() === forgotEmail.toLowerCase())) {
//       toast.error("No account with that email"); return;
//     }
//     const otp = generateOtp();
//     localStorage.setItem("aislemind:otp", JSON.stringify({ email: forgotEmail, otp, kind: "reset", at: Date.now() }));
...
//   const confirmOtp = () => {
//     if (!pending) return;
//     if (otpInput !== pending.otp) { toast.error("Incorrect code"); return; }
//     if (pending.kind === "login") {
//       const u = loginWithCredentials(pending.email, pending.password);
//       if (u) { toast.success(`Welcome back, ${u.name}`); setPending(null); routeFor(u); }
//     } else if (pending.kind === "signup") {
//       const u = signup(pending.data);
//       if (u) { setPending(null); routeFor(u); }
//     } else if (pending.kind === "reset") {
//       setResetOpen(true);
//       // keep pending for post-reset
//     }
//   };

//   const submitNewPwd = () => {
//     if (pending?.kind !== "reset") return;
//     if (newPwd.p1.length < 6) { toast.error("Min 6 characters"); return; }
//     if (newPwd.p1 !== newPwd.p2) { toast.error("Passwords don't match"); return; }
...
//       setNewPwd({ p1: "", p2: "" });
//       setMode("login");
//       setLoginForm({ email: pending.email, password: "" });
//     }
//   };
...
//             </div>
//             <h1 className="text-3xl font-bold tracking-tight">Welcome to Aislemind</h1>
...
//               <form onSubmit={startLogin} className="space-y-4 rounded-2xl border border-border bg-card p-6 shadow-soft">
//                 <div>
//                   <Label>Email</Label>
...
//                     <Input required type={showPwd ? "text" : "password"} value={loginForm.password}
...
//               <form onSubmit={startSignup} className="space-y-4 rounded-2xl border border-border bg-card p-6 shadow-soft">
//                 <div>
...
//                 <div>
//                   <Label>Email</Label>
//                   <Input required type="email" value={signupForm.email}
//                     onChange={(e) => setSignupForm({ ...signupForm, email: e.target.value })}
//                     className="mt-1.5" />
//                 </div>
//                 <div>
//                   <Label>Password</Label>
//                   <Input required type="password" value={signupForm.password}
//                     onChange={(e) => setSignupForm({ ...signupForm, password: e.target.value })}
//                     className="mt-1.5" placeholder="Min 6 characters" />
//                 </div>
//                 <div>
//                   <Label>Account type</Label>
//                   <select className="w-full mt-1.5 rounded-lg border border-input bg-background h-10 px-3 text-sm"
...
//                     <Input value={signupForm.storeName}
...
//             <Link to="/" className="hover:text-primary">← Back to marketplace</Link>
//           </p>
//         </div>

//         {/* Demo credentials column */}
//         <aside className="rounded-2xl border border-border bg-card p-5 shadow-soft">
//           <p className="text-xs uppercase tracking-wider font-semibold text-primary mb-3">Demo accounts</p>
//           <p className="text-xs text-muted-foreground mb-4">Use any of these to explore. OTP appears in a popup.</p>
//           <div className="space-y-2">
//             {DEMO_CREDS.map((c) => (
//               <button key={c.email}
//                 onClick={() => { setMode("login"); setLoginForm({ email: c.email, password: c.password }); }}
...
//                 <div className="flex justify-between items-center">
//                   <span className="text-[10px] uppercase tracking-wider font-bold text-primary">{c.role}</span>
//                   <span className="text-[10px] text-muted-foreground">click to fill</span>
//                 </div>
//                 <p className="text-sm font-mono mt-1 truncate">{c.email}</p>
//                 <p className="text-xs font-mono text-muted-foreground">{c.password}</p>
//               </button>
//             ))}
//           </div>
//         </aside>
...
//       {/* OTP popup */}
//       <Dialog open={!!pending && !resetOpen} onOpenChange={(o) => !o && setPending(null)}>
//         <DialogContent>
//           <DialogHeader>
//             <DialogTitle className="flex items-center gap-2"><Mail className="h-5 w-5 text-primary" /> Email verification</DialogTitle>
//             <DialogDescription>
//               We sent a 6-digit code to <span className="font-semibold text-foreground">{pending && (pending.kind === "signup" ? pending.data.email : pending.email)}</span>.
//             </DialogDescription>
//           </DialogHeader>
//           {pending && (
//             <div className="rounded-lg bg-primary-soft border border-primary/20 p-4 text-center">
//               <p className="text-xs uppercase tracking-wider text-primary font-bold mb-1">Demo OTP</p>
//               <p className="text-3xl font-bold font-mono tracking-[0.4em] text-primary">{pending.otp}</p>
//             </div>
//           )}
//           <div>
//             <Label>Enter the 6-digit code</Label>
//             <Input value={otpInput} onChange={(e) => setOtpInput(e.target.value.replace(/\D/g, "").slice(0, 6))}
//               className="mt-1.5 text-center text-xl font-mono tracking-[0.3em]" maxLength={6} />
//           </div>
//           <DialogFooter>
//             <Button variant="ghost" onClick={() => setPending(null)}>Cancel</Button>
//             <Button variant="hero" onClick={confirmOtp}>Verify</Button>
//           </DialogFooter>
//         </DialogContent>
//       </Dialog>
...
//             <DialogDescription>Enter your email to receive a verification code.</DialogDescription>
//           </DialogHeader>
//           <div>
//             <Label>Email</Label>
//             <Input type="email" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} className="mt-1.5" />
//           </div>
//           <DialogFooter>
//             <Button variant="ghost" onClick={() => setForgotOpen(false)}>Cancel</Button>
//             <Button variant="hero" onClick={startForgot}>Send code</Button>
//           </DialogFooter>
//         </DialogContent>
//       </Dialog>

//       {/* Reset password (post-OTP) */}
//       <Dialog open={resetOpen} onOpenChange={(o) => { setResetOpen(o); if (!o) setPending(null); }}>
//         <DialogContent>
//           <DialogHeader>
...
//               <Input type="password" value={newPwd.p1} onChange={(e) => setNewPwd({ ...newPwd, p1: e.target.value })} className="mt-1.5" />
//             </div>
//             <div>
//               <Label>Confirm password</Label>
//               <Input type="password" value={newPwd.p2} onChange={(e) => setNewPwd({ ...newPwd, p2: e.target.value })} className="mt-1.5" />
//             </div>
//           </div>
//           <DialogFooter>
//             <Button variant="ghost" onClick={() => { setResetOpen(false); setPending(null); }}>Cancel</Button>
//             <Button variant="hero" onClick={submitNewPwd}>Update password</Button>
//           </DialogFooter>
//         </DialogContent>
//       </Dialog>
//     </Shell>
